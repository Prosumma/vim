...
syn keyword     rebolMathFunction  minimum mod multiply negate power random remainder sine
...
syn keyword     rebolLogicFunction binary? block? char? date? decimal? equal? email? empty?
syn keyword     rebolLogicFunction file? found? function? integer? issue? logic? money?
syn keyword     rebolLogicFunction native? none? not-equal? object? paren? path? port? series?
syn keyword     rebolLogicFunction string? time? tuple? url? word?
syn keyword     rebolLogicFunction exists? input? same? value?

" Datatypes
syn keyword     rebolType       any-type! binary! block! char! date! datatype! decimal! email! file!
...
syn keyword     rebolType       series! tuple! url! word!
syn keyword     rebolTypeFunction type? to

" Control statements
syn keyword     rebolStatement  break case catch exit halt reduce return shield
syn keyword     rebolConditional if else either unless
syn keyword     rebolRepeat     for forall foreach forskip loop repeat while until do try

" Series statements
syn keyword     rebolStatement  ajoin append rejoin repend
syn keyword     rebolStatement  change clear closure copy fifth find first format fourth free
syn keyword     rebolStatement  func funct function head insert last match next parse past
syn keyword     rebolStatement  pick remove reverse second select skip sort tail third trim until length?
...
syn keyword     rebolOperator   size? modified? dir?

" Debug statement
syn keyword     rebolStatement  help probe trace

" Misc statements
syn keyword     rebolStatement  func function free set
